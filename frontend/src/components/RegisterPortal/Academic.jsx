...
import api, { academic, reports, classes } from '../../api';

// Component defined outside to prevent re-instantiation errors
...
const Academic = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [resAreas, resTerms, resWindows, resReports, resClasses] = await Promise.all([
        academic.getLearningAreas(),
        academic.getTerms(),
        academic.getAssessmentWindows(),
        reports.getAll(),
        classes.getAll()
      ]);
  ...
  const handleGenerateReport = async () => {
    ...
    setLoading(true);
    try {
      const response = await reports.generateBatch(reportFilters.class_id, reportFilters.term);
  ...
  const handleViewReport = async (report) => {
    try {
      const response = await reports.getById(report.id);
              ...
              <table class="performance-table">
                ...
                <tbody>
                  ${data.learning_area_performance && data.learning_area_performance.length > 0 ? 
                    ...
                    '<tr><td colspan="3">No learning area data available</td></tr>'
                  }
                </tbody>
              </table>
            </div>
  ...
  const handlePublishReport = async (report) => {
    ...
    try {
      await reports.publish(report.id);
  ...
  const handleBulkDownload = async () => {
    ...
    try {
      const response = await api.post('/api/cbe-report-cards/batch_download/', {

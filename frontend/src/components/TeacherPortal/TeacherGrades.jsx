      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {classes.map(cls => (
          <div key={cls.id} className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
            ...
            <Link
              to={`/teacher/classes/${cls.id}?tab=grades`}
              className="mt-3 inline-block bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700 text-sm"
            >
              <FiBookOpen className="inline mr-1" /> Enter Grades
            </Link>
          </div>
        ))}
      </div>

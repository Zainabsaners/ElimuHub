...
const PaymentRecords = () => {
  ...
  const printReceiptFromRecords = async (transaction) => {
  if (!transaction) return;

  try {
    await axios.patch(`${API_BASE_URL}/api/fees/transactions/${transaction.id}/mark_as_printed//`, {}, getAuthHeaders());
  ...
  const outstanding = parseFloat(transaction.student_balance || 0).toLocaleString('en-KE', { minimumFractionDigits: 2 });
  const paid = parseFloat(transaction.amount_kes || 0).toLocaleString('en-KE', { minimumFractionDigits: 2 });

  printWindow.document.write(`
    <html>
      <head>
        <style>
          .receipt { border: 2px solid #b91c1c; padding: 30px; font-family: sans-serif; max-width: 500px; margin: auto; }
          .header { text-align: center; border-bottom: 2px solid #b91c1c; margin-bottom: 20px; padding-bottom: 10px; }
          .school-name { color: #b91c1c; font-size: 24px; font-weight: bold; margin: 0; }
      ...
      <body>
        <div class="receipt">
          ...
          <div class="row"><span>Receipt No:</span><b>${transaction.transaction_no}</b></div>
          <div class="row"><span>Student:</span><span>${transaction.student_name}</span></div>
          <div class="row"><span>Admission No:</span><span>${transaction.admission_no}</span></div>
          <div class="row"><span>Date:</span><span>${new Date(transaction.payment_date).toLocaleDateString()}</span></div>
          <div class="row"><span>Mode:</span><span>${transaction.payment_mode}</span></div>
          <hr/>
          <div class="row" style="font-size: 1.2em;"><span>Amount Paid:</span><b style="color: green;">KSh ${paid}</b></div>
          <div class="row" style="font-size: 1.2em; background: #fff1f2; padding: 10px;">
             <span>OUTSTANDING BALANCE:</span><b class="red">KSh ${outstanding}</b>
          </div>
          <div class="footer">
            <p>© ${new Date().getFullYear()} ELIMUHUB • Generated by Bursar Portal</p>
          </div>
        </div>
